...
- `readEDF()` **EDF+ annotation fix**: a data record's annotation channel holds
  one or more Time-stamped Annotation Lists, each terminated by a NUL (`0x00`),
  with the channel NUL-padded to its fixed length. The reader stripped every NUL
  before parsing, which destroyed the TAL boundaries, and then split on
  `0x14 0x14`, which is not a TAL delimiter. Records holding more than one event
  TAL were therefore fused into a **single** annotation whose value was every
  label concatenated. The Sleep-EDF Expanded hypnograms, which store a whole
  night of scoring in one data record, collapsed from **154 annotations to 1**
  (and 126 to 1 on the second night). The channel is now split on its NUL
  separators and each TAL parsed individually: both hypnograms reproduce
  `mne.read_annotations()` exactly (154 and 126 events, onsets and durations
  identical). Files that carry one TAL per record — the BCI2000/eegmmidb EDF+
  recordings, for instance — parsed correctly before and are unchanged
  (re-verified against MNE).

  The defect could not be reached by a round trip, because `writeEDF()` writes
  no annotation channel at all; like the `readWFDB()` baseline defect fixed in
  0.2.7, it needed a comparison against the format's reference implementation to
  surface. A regression test now builds a synthetic single-record EDF+ holding
  three event TALs and asserts they stay separate.

# PhysioIO 0.2.7

## Bug Fixes

- `readWFDB()` **data-correctness fix**: a signal line whose gain field has no
  explicit `(baseline)` annex — the style of the original MIT-BIH headers
  (`... 212 200 11 1024 995 ...`) — was read with `baseline = gain` instead of
  the header(5) default `baseline = ADC zero`, because the extracting `sub()`
  returns its input unchanged on a non-match. Every physical value gained a
  constant `(adczero - gain)/gain` offset: **+4.12 mV on MIT-BIH** (gain 200,
  ADC zero 1024) and exactly **-1 physical unit on Fantasia-style headers**
  (bare gain, ADC zero 0). Headers that carry `(baseline)` (BIDMC, GRABMyo,
  Non-EEG, and everything written by `writeWFDB()`, which always emits the
  annex — the reason round-trip tests never caught this) were unaffected.
  Downstream, the offset degraded amplitude-sensitive processing: the default
  Pan-Tompkins detector missed beats on offset MIT-BIH signals that it finds
  cleanly in correct units. Adds bare-gain / explicit-baseline / no-ADC-zero
  regression tests, and replaces the synthetic `extdata` record-100 fixture
  with a genuine first-5000-sample excerpt of PhysioNet MIT-BIH record 100
  (ODC-By v1.0) that keeps the original bare-gain header style so the
  baseline-default rule stays regression-tested against real bytes.
