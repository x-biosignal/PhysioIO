...
test_that("bare gain field defaults baseline to the ADC zero (original MIT-BIH headers)", {
  # mitdb-style signal line: gain '200' with NO '(baseline)' annex and ADC zero
  # 1024. header(5): a missing baseline defaults to the ADC zero. The old code
  # extracted with an unconditional sub(), whose non-match returns the input,
  # so baseline silently became gain (200) and every value gained
  # (1024-200)/200 = +4.12 mV.
  d <- tempfile()
  raw_adu <- c(995L, 1024L, 1053L)                 # around the 1024 ADC zero
  writeBin(writeBin(raw_adu, raw(), size = 2L, endian = "little"), paste0(d, ".dat"))
  writeLines(c(
    sprintf("%s 1 360 3", basename(d)),
    sprintf("%s.dat 16 200 11 1024 995 0 0 MLII", basename(d))),
    paste0(d, ".hea"))
  x <- readWFDB(d)
  unlink(paste0(d, c(".hea", ".dat")))
  expect_equal(as.numeric(SummarizedExperiment::assay(x, "raw")[, 1]),
               (raw_adu - 1024) / 200)             # -0.145, 0, +0.145 mV
})

test_that("bare gain with no ADC-zero field defaults baseline to 0 (Fantasia-style)", {
  d <- tempfile()
  raw_adu <- c(-100L, 0L, 100L)
  writeBin(writeBin(raw_adu, raw(), size = 2L, endian = "little"), paste0(d, ".dat"))
  writeLines(c(
    sprintf("%s 1 250 3", basename(d)),
    sprintf("%s.dat 16 2000 16 0 -100 0 0 ECG", basename(d))),
    paste0(d, ".hea"))
  x <- readWFDB(d)
  unlink(paste0(d, c(".hea", ".dat")))
  # old code: baseline = gain = 2000 -> every value shifted by exactly -1 unit
  expect_equal(as.numeric(SummarizedExperiment::assay(x, "raw")[, 1]),
               raw_adu / 2000)
})

test_that("an explicit (baseline) annex still wins over the ADC zero", {
  d <- tempfile()
  raw_adu <- c(0L, 512L)
  writeBin(writeBin(raw_adu, raw(), size = 2L, endian = "little"), paste0(d, ".dat"))
  writeLines(c(
    sprintf("%s 1 100 2", basename(d)),
    sprintf("%s.dat 16 100(512)/uV 12 999 0 0 0 X", basename(d))),
    paste0(d, ".hea"))
  x <- readWFDB(d)
  unlink(paste0(d, c(".hea", ".dat")))
  expect_equal(as.numeric(SummarizedExperiment::assay(x, "raw")[, 1]),
               (raw_adu - 512) / 100)              # baseline 512, NOT adczero 999
})

test_that("the bundled record-100 fixture reads in correct physical units", {
  rec <- sub("\\.hea$", "", system.file("extdata", "100.hea", package = "PhysioIO"))
  skip_if(rec == "" || !file.exists(paste0(rec, ".hea")), "fixture not installed")
  x <- readWFDB(rec)
  d <- SummarizedExperiment::assay(x, "raw")
  # first stored MLII sample is ADC 995 with gain 200 / ADC zero 1024:
  expect_equal(unname(d[1, 1]), (995 - 1024) / 200)  # -0.145 mV, not +3.975
  # an ECG excursion averages near 0 mV; the bug put the mean near +4 mV
  expect_lt(abs(mean(d[, 1])), 0.5)
})
