...
.parse_hea <- function(path) {
  ...
  for (i in seq_len(nsig)) {
    ...
    # baseline is the "(n)" annex of the gain field; when absent it defaults to
    # the ADC zero (header(5)). sub() returns its input UNCHANGED on a
    # non-match, so extracting unconditionally turned a bare gain ("200", as in
    # the original MIT-BIH headers) into baseline = gain, offsetting every
    # physical value by (adczero - gain)/gain (+4.12 mV on mitdb).
    baseline <- if (grepl("(", gain_field, fixed = TRUE)) {
