...
.parseEDFAnnotations <- function(con, signal_headers, n_records, annot_idx) {
  ...
  for (rec in seq_len(n_records)) {
    ...
    # In EDF+ a data record's annotation channel holds one or more TALs, each
    # terminated by a NUL (0x00); trailing NULs pad the channel. Stripping every
    # NUL therefore destroys the TAL boundaries and fuses the whole record into a
    # single annotation -- on files that consolidate a night of scoring into one
    # data record (the Sleep-EDF hypnograms) that collapsed 154 events into 1.
    # Split on the NUL separators and keep each TAL intact.
    n_bytes <- signal_headers$samples_per_record[annot_idx] * 2
    raw_annot <- readBin(con, "raw", n_bytes)
    annot_str <- .splitTALs(raw_annot)

    # Parse each TAL (Time-stamped Annotation List) of this data record
    for (tal in annot_str) {
      parsed <- .parseTAL(tal)
      if (nrow(parsed) > 0) {
...
#' Split one EDF+ annotation channel into its TALs
#'
#' Each Time-stamped Annotation List in a data record's annotation channel is
#' terminated by a NUL byte (0x00), and the channel is NUL-padded to its fixed
#' length. R strings cannot carry an embedded NUL, so the split is done on the
#' raw bytes. Returns one character string per TAL, padding dropped.
#' @noRd
.splitTALs <- function(raw_annot) {
  n <- length(raw_annot)
  if (n == 0L) return(character(0))
  nul <- which(raw_annot == as.raw(0L))
  starts <- c(1L, nul + 1L)
  ends   <- c(nul - 1L, n)
  keep   <- starts <= ends & starts <= n
  vapply(which(keep), function(i) {
    seg <- raw_annot[starts[i]:min(ends[i], n)]
    if (length(seg) == 0L) "" else rawToChar(seg)
  }, character(1))
}
